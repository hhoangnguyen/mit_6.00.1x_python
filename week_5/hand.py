...
class Hand(object):
    ...
    def update(self, word):
        ...
        """
        for char in word:
            # if char is not a hand's key or we run out of char key, not valid
            if char not in self.hand.keys() or self.hand[char] == 0:
                return False

        # we can now update hand safely
        for char in word:
            self.hand[char] = self.hand.get(char, 0) - 1
...
myHand = Hand(10)
myHand.setDummyHand('ehhheqiehb')
myHand.update('teeth')
print(myHand)
